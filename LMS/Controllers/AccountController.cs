...
namespace LMS.Controllers
{
  ...
  public class AccountController : CommonController
  {
    ...
    {
            string uid = findNextUID();

            try
            {
                if (role == "Student")
                {
                    uint dID = getDID(SubjectAbbrev);
    ...
    private string findNextUID()
    {
            ...
            var admins =
                      ...
                      select new
                      {
                          uid = a.UId.Substring(1)
                      };

            
            foreach (var s in students)
            {
                if (int.Parse(s.uid) >= maxUID)
                    maxUID = int.Parse(s.uid);
            }

            foreach (var p in professors)
            {
                if (int.Parse(p.uid) >= maxUID)
                    maxUID = int.Parse(p.uid);
            }

            foreach (var a in admins)
            {
                if (int.Parse(a.uid) >= maxUID)
                    maxUID = int.Parse(a.uid);
            }

            maxUID++;

            string uid = "u" + maxUID.ToString("D7");
